...
import com.dinkydetails.telly.R;
import com.dinkydetails.telly.R.id;
import com.dinkydetails.telly.R.layout;
...
public class MainActivity extends Activity {
	...
	{
		//Grab the intent (phone number) and convert it to a string
		String strReceivedPhNo = getIntent().getDataString();
		
		if(strReceivedPhNo != null)
		{
			//Log the text to verify its contents
			Log.v("","strReceivedPhNo: "+strReceivedPhNo);
			//Since Web is usually formated "Tel: XXXX" need to break up the string to only grab the numbers
			String parts [] = strReceivedPhNo.split(":");
			//Decode the left over string to make sure it is only text and nothing left. 
			String strphNo = URLDecoder.decode(parts[1]);
			
			//log the number that was received
		    Log.v("","strphNo: "+strphNo);
		    //take the number received and put it into the text view
		    ((TextView)findViewById(R.id.tvNumber)).setText(strphNo);
		}
	}
